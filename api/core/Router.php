...
class Router
{
  ...
  public function handleRequest()
  {
    ...
    // TODO:validar que no pasen cosas raras con rutas con slashes raros , curar la entrada
    ...
    if (isset($endpoint)) {
      ...
      // Routing -------------------------------------------------------
      $controllerClass = $endpoint->controller;
      $controllerInstance = new $controllerClass();

      $method2Call = $endpoint->method;
      if (method_exists($controllerInstance, $method2Call)) {
        return $controllerInstance->{$method2Call}();
      } else {
        $statusCode = 500;
        ...
        if ($GLOBALS['config']->apiMode == "debug") {
          $message = 'El controlador: "' . ($controllerClass) .  "\" no tiene el metodo: \"$method2Call()\"";
        }
      }
    }
